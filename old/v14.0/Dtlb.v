module Dtlb (clk,rstn,addr_phys,cr3,cr0,data_miss,iDaddr,pg_en,iwrite_data,owrite_data,
iread_req,iread_ack,iwrite_req,iwrite_ack,iread_sz,oread_sz, iwrite_sz , owrite_sz,
oread_req,oread_ack,owrite_req,owrite_ack, 
pg_fault, wr_fault, cr2, flush_tlb, cs, pt_fault , busy_ram, outstanding
);

input         clk,rstn;
output reg [31:0] addr_phys,owrite_data;
input  [31:0] iDaddr,data_miss,iwrite_data;
input  [31:0] cr3,cr0,cs;
input         iread_req,iread_ack,iwrite_req,iwrite_ack;
output reg    oread_req,owrite_req;
output        oread_ack,owrite_ack;
...
output reg [31:0] cr2;
input             busy_ram;
input       [1:0] iread_sz,iwrite_sz;
output reg  [1:0] oread_sz,owrite_sz;
output            outstanding;
...
assign outstanding = ((fsm == 6)||(fsm==7)||(fsm==8)||(fsm==9)) ? 1:0;


always @(posedge clk or negedge rstn)
if (rstn == 0) owrite_req <=0; else  owrite_req <= (pg_en == 0) ? iwrite_req^iwrite_ack : 
                                                   (( (iwrite_req==1) & (fsm == 0) & (hit  == 1) )|((fsm == 6)|(fsm==7))) ^ iwrite_ack ;

always @(posedge clk or negedge rstn)
if (rstn == 0) owrite_sz <=0; else  owrite_sz <= ((fsm == 6)|(fsm == 7)) ? 2'b00 : iwrite_sz ;

always @(posedge clk or negedge rstn)
if (rstn == 0) oread_req <=0; else  oread_req <= (pg_en == 0) ? iread_req ^ iread_ack : ( req_miss | (iread_req  & (fsm == 0) & (hit  == 1) )) ^ iread_ack;

always @(posedge clk or negedge rstn)
if (rstn == 0) addr_phys <=0; else
      addr_phys <= (pg_en == 0) ? iDaddr : 
                  ((fsm == 6)||(fsm==7)) ? wrA :
                  ((hit   == 1)&~((fsm == 6)|(fsm==7))) ? {dir_mux,iDaddr[11:0]} : 
		                  addr_miss;

always @(posedge clk or negedge rstn)
if (rstn == 0) oread_sz <=0; else oread_sz  <= (pg_en == 0) ? iread_sz   :  ((fsm == 0) & (hit == 1) ) ? iread_sz : 2'b10 ;
				  
always @(posedge clk or negedge rstn)
if (rstn == 0) owrite_data <=0; else owrite_data <= ((fsm == 6)|(fsm == 7)) ? wrD : iwrite_data;
