...
s00_AXI_CLK,
// v spi 0,1,2
mosi_0,miso_0,sclk_0,spi_csn0,
mosi_1,miso_1,sclk_1,spi_csn1,
mosi_2,miso_2,sclk_2,spi_csn2,

// AXI4 LITE 32 BIT BUS
s00_AXI_AWADDR, 
s00_AXI_AWVALID, 
s00_AXI_AWREADY, 
s00_AXI_AWBURST, 
...
//  tiny spi
output mosi_0,mosi_1,mosi_2;
input  miso_0,miso_1,miso_2;
output sclk_0,sclk_1,sclk_2;
output spi_csn0,spi_csn1,spi_csn2;
...
// spi wires
wire [7:0] dat_o_spi_0,dat_o_spi_1,dat_o_spi_2;
wire cs_spi_0,cs_spi_1,cs_spi_2;
wire we_spi_0,we_spi_1,we_spi_2;
reg [3:0] int_reg;
reg [2:0] spi_csn_reg;
assign {spi_csn2,spi_csn1,spi_csn0} = spi_csn_reg;
...
assign readio_data = (io_add[15:2] == 14'b0000_0000_0100_00  ) ? rdio_pit  :
	                 ...
	                 ({io_add[15:4],4'b0} == 16'h0100) ? {24'b0,dat_o_spi_0} :
	                 ({io_add[15:4],4'b0} == 16'h0110) ? {24'b0,dat_o_spi_1} :
	                 ({io_add[15:4],4'b0} == 16'h0120) ? {24'b0,dat_o_spi_2} :
			          32'hffff_ffff;
...
if (s00_AXI_RSTN == 0) 
 begin 
  ...
  if ((io_add[15:0]==16'h0291) && (s00_AXI_WREADY == 1)) superIOb <= writeio_data[7:0]; 
  end
...
// External Chip select registers GPIOs
always @(posedge s00_AXI_CLK or negedge s00_AXI_RSTN)
if (s00_AXI_RSTN == 0) 
  spi_csn_reg  <= 3'b111;
else
  begin 
  if ((io_add[15:0]==16'h0605) && (s00_AXI_WREADY == 1) && (s00_AXI_WVALID == 1)) spi_csn_reg[0] <= writeio_data[0]; else
  if ((io_add[15:0]==16'h0615) && (s00_AXI_WREADY == 1) && (s00_AXI_WVALID == 1)) spi_csn_reg[1] <= writeio_data[0]; else
  if ((io_add[15:0]==16'h0625) && (s00_AXI_WREADY == 1) && (s00_AXI_WVALID == 1)) spi_csn_reg[2] <= writeio_data[0]; 
  end

assign cs_spi_0 =(({io_add[15:4],4'b0} == 16'h0100 ) && (s00_AXI_WREADY == 0)) ? 1'b1 : 1'b0;	      
assign we_spi_0 =(({io_add[15:4],4'b0} == 16'h0100 ) && (s00_AXI_WREADY == 1)) ? 1'b1 : 1'b0;

vspi uspi_0 (.rstn_i(s00_AXI_RSTN), .clk_i(s00_AXI_CLK), .stb_i(cs_spi_0|we_spi_0), .we_i(we_spi_0), .dat_o(dat_o_spi_0), .dat_i({writeio_data[7:0]}), .adr_i(io_add[2:0]), .MOSI(mosi_0), .SCLK(sclk_0) , .MISO(miso_0) , .int_o() );

assign cs_spi_1 =(({io_add[15:4],4'b0} == 16'h0110 ) && (s00_AXI_WREADY == 0)) ? 1'b1 : 1'b0;	      
assign we_spi_1 =(({io_add[15:4],4'b0} == 16'h0110 ) && (s00_AXI_WREADY == 1)) ? 1'b1 : 1'b0;

vspi uspi_1 (.rstn_i(s00_AXI_RSTN), .clk_i(s00_AXI_CLK), .stb_i(cs_spi_1|we_spi_1), .we_i(we_spi_1), .dat_o(dat_o_spi_1), .dat_i({writeio_data[7:0]}), .adr_i(io_add[2:0]), .MOSI(mosi_1), .SCLK(sclk_1) , .MISO(miso_1) , .int_o() );

assign cs_spi_2 =(({io_add[15:4],4'b0} == 16'h0120 ) && (s00_AXI_WREADY == 0)) ? 1'b1 : 1'b0;	      
assign we_spi_2 =(({io_add[15:4],4'b0} == 16'h0120 ) && (s00_AXI_WREADY == 1)) ? 1'b1 : 1'b0;

vspi uspi_2 (.rstn_i(s00_AXI_RSTN), .clk_i(s00_AXI_CLK), .stb_i(cs_spi_2|we_spi_2), .we_i(we_spi_2), .dat_o(dat_o_spi_2), .dat_i({writeio_data[7:0]}), .adr_i(io_add[2:0]), .MOSI(mosi_2), .SCLK(sclk_2) , .MISO(miso_2) , .int_o() );
