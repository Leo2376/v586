...
read_req  , read_ack  , read_data  , Daddr , busy_ram );
...
output [31:0] write_data,Daddr,writeio_data,io_add,code_wdata;
input  [31:0] read_data,readio_data;
...
wire         pg_fault,ipg_fault,wr_fault,pt_fault,flush_Dtlb;
wire   [1:0] read_sz_tlb,int_write_sz;
wire   [5:0] valid_len;


wire [31:0] Daddr_realign, read_data_realign, write_data_realign;
...
useq i_useq (.clk(clk), .rstn(rstn),
	     ...
	     .pc_in(pc_out), .pc_req(pc_req), .pg_fault(ipg_fault), .busy_ram(busy_ram)
	     );
...
Dtlb i_Dtlb (.clk(clk), .rstn(rstn) ,  .cr3(cr3), .cr0(cr0), .pg_en(pg_en),.cs(cs),
	  ...
	  .pg_fault(pg_fault), .cr2(cr2), .wr_fault(wr_fault) , .pt_fault(pt_fault) , .busy_ram(busy_ram) );
