module psram_core #(parameter SRAM_DATA_WIDTH = 32,
           ...
           `IDLE: begin
              if (cyc_i && stb_i && ~ack_o)
                if (we_i) begin            
                   ...
                   // classic cycle or if data width < 32 , also on cellular, sram page burst is not possible accross 16 words boundary in case of 16bit width
                end else if (cti_i == 3'b000 || cti_i == 3'b111 || cti_i == 3'b010 || !sram_data_width_32 ) begin 
                   we_bst <= 1'b0;
                   rd_bst <= 0;
                   sram_addr_bst     <= 0;                 
