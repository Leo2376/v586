...
assign read_data = (fsm == 5'b11110) ? axi_R :
                   (A4 == 2'b00) ? cacheQ[ 31: 0] :
                   (A4 == 2'b01) ? cacheQ[ 63:32] :
		           (A4 == 2'b10) ? cacheQ[ 95:64] :
		                           cacheQ[127:96] ;
assign cacheA = Daddr[13:4];
...
9'b00001_xxxx : begin
		     ...
		     axi_AW      <= A32; 
		     axi_WLAST   <= 0;
		     if ((axi_AW[31:14] == cacheQ[145:128]) && (cacheQ[148] == 1)) begin cacheD <= 0; cacheWEN <= 0; end // cache devalidate 
		   end  	 
...
9'b01011_01xx : begin
                 if (axi_AR[31:30] == 2'b01) // 0x4000_0000 is uncached
                 begin
                    fsm <= 5'b01100; 
                    axi_ARVALID <= 1;   
                 ...
                 if ((axi_AR[31:14] == cacheQ[145:128]) && (cacheQ[148] == 1)) 
                   begin 
                    read_ack_slow<=1; 
                    fsm <= 5'b11111; 
                   end 
...
9'b01000_xxxx : begin
	          ...
	          begin 
	           ...
	           if (burst_idx == 0) cacheD[ 31:  0]<=axi_R; else
	           if (burst_idx == 1) cacheD[ 63: 32]<=axi_R; else
	           if (burst_idx == 2) cacheD[ 95: 64]<=axi_R; else
	           if (burst_idx == 3) cacheD[127: 96]<=axi_R; 
		   if (axi_RLAST) 
		      begin 
		       fsm<=5'b01001; 
		       cacheD[149:128] <= {2'b01,2'b0,Daddr[31:14]};
		       cacheWEN <= 0;  
		       axi_RREADY    <= 0;
		      end
	          end
...
9'b00000_001x : begin
		 ...
		 if (code_addr[1:0]==2'b00) fsm       <= 5'b10001; else fsm       <= 5'b10000;
		end
