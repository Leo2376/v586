...
module tlb (clk,rstn,addr_phys,cr3,cr0,data_miss,iDaddr,pg_en,iwrite_data,owrite_data,
...
if (rstn == 0) owrite_req <=0; else  owrite_req <= (pg_en == 0) ? iwrite_req :             ((iwrite_req & (fsm == 0) & (hit  == 1) )|((fsm == 6)|(fsm==7))) ;

always @(posedge clk or negedge rstn)
if (rstn == 0) oread_req <=0; else  oread_req = (pg_en == 0) ? iread_req  : ( req_miss | (iread_req  & (fsm == 0) & (hit  == 1) ));
...
always @(posedge clk or negedge rstn)
if (rstn == 0) owrite_data; else owrite_data <= ((fsm == 6)|(fsm == 7)) ? wrD : iwrite_data;
...
if (rstn == 0)
 ...
 begin
  ...
  fsm5_cnt <=0;
    ...
    begin
     if (fsm5_cnt<180) fsm5_cnt <= fsm5_cnt + 1; else begin fsm <= 0; pg_fault <= 0; wr_fault <= 0; end
    end         
   else if (fsm ==6)
    begin
