//==========================================================================
//  tb_top_sim.cpp  --  Verilator C++ testbench for the v10.8.2 TOP_SYS_sim
//
//  Generates a 100 MHz clock, holds the CPU in reset, then runs for
//  --cycles cycles (default 200000). Prints EIP milestones, the v586
//  debug port and UART TXD transitions to watch boot progress.
//
//  Options:
...
static void tick(VTOP_SYS_sim* dut) {
    dut->clk = 0; dut->eval(); sim_time += clk_period_ns / 2;
    dut->clk = 1; dut->eval(); sim_time += clk_period_ns / 2;
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--cycles") && i + 1 < argc) max_cycles = strtoull(argv[++i], nullptr, 10);
    ...
    dut->clk = 0; dut->rstn = 0; dut->RXD = 1;
    for (int i = 0; i < 16; i++) { tick(dut); if (tfp) tfp->dump(sim_time); }
    dut->rstn = 1;
    vluint64_t cyc = 0;
    uint8_t   last_txd = 1, last_dbg = 0;
    uint32_t  last_eip = 0xffffffff, eip_changes = 0, high_seen = 0;
    vluint64_t stuck_from = 0;
    while (cyc < max_cycles) {
        tick(dut);
        if (tfp) tfp->dump(sim_time);
        if (dut->TXD != last_txd) {
            printf("[t=%llu cyc=%llu] TXD %d->%d\n",
                   (unsigned long long)sim_time, (unsigned long long)cyc, last_txd, dut->TXD);
            last_txd = dut->TXD;
        }
        ...
        if (dut->dbg_eip != last_eip) {
            eip_changes++; stuck_from = cyc;
            if (dut->dbg_eip >= 0xc0000000) high_seen = 1;
            if (verbose || eip_changes <= 80 || (eip_changes % 50000 == 0)) {
                printf("[cyc=%llu #%u] EIP %08x -> %08x  ecx=%08x daddr=%08x pgf=%d pcpgf=%d\n",
                       (unsigned long long)cyc, eip_changes, last_eip, dut->dbg_eip,
                       dut->dbg_ecx, dut->dbg_Daddr, dut->dbg_pg_fault, dut->dbg_pc_pg_fault);
            }
